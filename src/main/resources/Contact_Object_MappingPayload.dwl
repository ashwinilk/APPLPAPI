...
(vars.registrantMainPayload) map ((item, index) -> 
{
    ...
    "LastName" : item.lastName default "Singh",
    "Email" : item.email,
    ...
    "Country__c": (vars.registrantMainPayload filter $.registrantKey == item.registrantKey)."country"[0],
	  "Webinar_Organization__c": (vars.registrantMainPayload filter $.registrantKey == item.registrantKey)."organization"[0],
	   "Registration_Source_Webinar__c": "GTW",
	    "Interest_Rating_During_Webinar__c": "Good",
